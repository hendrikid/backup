...
<html>
    ...
    <head>
        ...
        <title>TK Tarbiyatul Fallah | Data User</title>
    ...
    <body style="background-image: url(image/bg-1.png); background-size:inherit;">
        <?php 
        if (isset($_GET['pesan'])) {
            if ($_GET['pesan']=="disable") { ?>
            <script>
            $( document ).ready(function() {
                swal({
                    title: '<h5>Akun berhasil diblokir.</h5>',
                    type: 'success',
        ...
        <?php
        } else if ($_GET['pesan']=="active") { ?>
            <script>
            $( document ).ready(function() {
                swal({
                    title: '<h5>Akun berhasil aktifkan.</h5>',
                    type: 'success',
                    showConfirmeButton: true
                })
                            ...
                            <div class="col-xl-12">
                                <div class="card text-white bg-light border-dark">
                                    <div class="card-body text-dark text-center">
                                        <div class="card-body">
                                                <h3 class="text-center" style="margin-top: 1%;">DATA USER</h3>
                                                <hr class="mb-2 mt-0 d-inline-block mx-auto" style="width: 60px; background-color: black; height: 2px;"/>
                                                <div class="table-responsive">
                                                    <?php 
                                                        include 'koneksi.php';
                                                        $no = 1;
                                                        $query = mysqli_query($koneksi,"SELECT user.id_user, user.username, user.level, pendaftar.nama_lengkap FROM user INNER JOIN pendaftaran ON pendaftaran.id_user=user.id_user INNER JOIN pendaftar ON pendaftaran.id_pendaftar=pendaftar.id_pendaftar");
                                                                ...
                                                                while($d = mysqli_fetch_array($query)){
                                                                    ...
                                                                    <tr>
                                                                        ...
                                                                        <?php 
                                                                            ...
                                                                            if($d['level']=='pengguna'){
                                                                                echo "<td>";
                                                                                    echo "<a href='auth_akses.php?id_1=$d[id_user]' class='btn btn-danger form-control'>";
                                                                                    echo "DISABLE</a>";
                                                                                echo "</td>";
                                                                            } else {
                                                                                echo "<td>";
                                                                                    echo "<a href='auth_akses.php?id_2=$d[id_user]' class='btn btn-primary form-control'>";
                                                                                    echo "ACTIVE</a>";
                                                                                echo "</td>";
                                                                            }
                                                                        ?>
