                        ...
                        <ul class="nav navbar-nav ml-auto text-dark">
                            <li>
                                <?php 
                                    ...
                                    $query = mysqli_query($koneksi,"SELECT pendaftar.id_pendaftar, pendaftar.nama_lengkap from pendaftar INNER JOIN pendaftaran ON pendaftaran.id_pendaftar=pendaftar.id_pendaftar INNER JOIN user ON user.id_user=pendaftaran.id_user WHERE user.username='$_SESSION[username]'");
                                            ...
                                            <div class="card bg-transparent p-2 border-dark" style="margin-bottom: 1%; border-radius: 15px 50px 30px 5px;">
                                                <div class="col-md-12">
                                                    <?php 
                                                        ...
                                                        $query_bayar = mysqli_query($koneksi,"SELECT * FROM pembayaran WHERE id_pendaftar='$nama_user[id_pendaftar]'");
                                                        $data_bayar = mysqli_fetch_array($query_bayar);

                                                        if ($data_bayar == NULL){?>
                                                            <h3 class="text-center" style="margin-top: 1%;">UPLOAD BUKTI BAYAR</h3>
                                                            <hr class="mb-2 mt-0 d-inline-block mx-auto container col-md-8" style="width: 60px; background-color: black; height: 2px;"/>
                                                            <form method="POST" enctype="multipart/form-data" action="auth_upload.php">
                                                                <div class="form-group row container" style="padding: 1px; margin-top: 1%">
                                                                    <input class="col-md-4" type="file" name="gambar">
                                                                    <input class="col-md-4" type="text" name="id_pendaftar" value="<?php echo $nama_user['id_pendaftar']; ?>" hidden>
                                                                    <input class="col-md-2" type="submit" value="Upload">
                                                                </div>
                                                            </form>
                                                    <?php        
                                                        } else {?>
                                                            <h3 class="text-center" style="margin-top: 1%;">PEMBAYARAN</h3>
                                                            <hr class="mb-2 mt-0 d-inline-block mx-auto container col-md-8" style="width: 60px; background-color: black; height: 2px;"/>
                                                            <table class="table table-bordered">
                                                                ...
                                                                <tbody>
                                                                    <tr>
                                                                        <td >1</td>
                                                                        <td ><?php echo $data_bayar['id_pendaftar'] ?></td>
                                                                        <td ><?php echo $data_bayar['tgl_bayar'] ?></td>
                                                                        <td ><?php echo $data_bayar['bukti_bayar'] ?></td>
                                                                        <td ><a href='edit_bayar.php?id_bayar=<?php echo $data_bayar['id_bayar']; ?>' class="btn btn-secondary">Edit</a></td>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                                    <?php
